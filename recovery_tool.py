...
def run_cmd(cmd: List[str], dry_run: bool = False, verbose: bool = False) -> CommandResult:
    started = utc_now()
    if verbose:
        print("$", " ".join(cmd))
    if dry_run:
        return CommandResult(
            ...
            returncode=0,
            stdout="",
            stderr="DRY_RUN: command not executed",
            started_at=started,
            ended_at=utc_now(),
        )
...
def attempt_fs_repair(target: Path, dry_run: bool, verbose: bool = False) -> List[dict]:
    ...
    if system == "darwin" and command_exists("fsck_apfs"):
        res = run_cmd(["fsck_apfs", "-n", str(target)], dry_run=dry_run, verbose=verbose)
        ops.append({"tool": "fsck_apfs", **asdict(res)})
    elif command_exists("fsck"):
        res = run_cmd(["fsck", "-N", str(target)], dry_run=dry_run, verbose=verbose)
...
def clone_source(
    ...
    if command_exists("ddrescue"):
        first_pass = run_cmd(
            ["ddrescue", "-f", "-n", source, str(image_path), str(map_path)],
            dry_run=dry_run,
            verbose=verbose,
        )
        retry_pass = run_cmd(
            ["ddrescue", "-d", f"-r{retries}", source, str(image_path), str(map_path)],
            dry_run=dry_run,
            verbose=verbose,
        )
        return {
    ...
    res = run_cmd(dd_cmd, dry_run=dry_run, verbose=verbose)
    return {
        "tool": "dd",
        "clone": asdict(res),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--source", help="Damaged source block device (example: /dev/disk6)")
    ...
    p.add_argument(
        "--mode",
        choices=["full", "extract-only"],
        default="full",
        help="full: clone + check (+optional extraction). extract-only: only copy files from --mount-source",
    )
    ...
    p.add_argument("--show-commands", action="store_true", help="Print underlying system commands before execution")
    p.add_argument("--dry-run", action="store_true", help="Plan and log actions without executing tools")
    return p.parse_args()


def validate_args(args: argparse.Namespace) -> None:
    if args.mode == "full" and not args.source:
        raise RecoveryError("--source is required in --mode full")
    if args.mode == "extract-only" and not args.mount_source:
        raise RecoveryError("--mount-source is required in --mode extract-only")
...
def main() -> int:
    ...
    try:
        if args.mode == "full":
            ...
            report["stages"]["repair_check"] = {
                "operations": attempt_fs_repair(image_path, dry_run=args.dry_run, verbose=args.show_commands),
                "note": "Checks are performed only against cloned image",
            }
        else:
            report["stages"]["clone"] = {"skipped": True, "reason": "mode is extract-only"}
            report["stages"]["repair_check"] = {"skipped": True, "reason": "mode is extract-only"}
    ...
    if args.dry_run:
        print("Dry run mode: no clone/check commands were executed.")
